...
async def get_message_hero(event):
	...
	if re.search(r'Битва семи замков через ?((\d+)ч\.)?( (\d+) ?(мин\.|минуты|минуту))?!', event.raw_text):
		...
		# if hours or minutes are None, catch exception and put in 0
		try:
			hours = int(re.search(r'Битва семи замков через ?((\d+)ч\.)?( (\d+) ?(мин\.|минуты|минуту))?!', event.raw_text).group(2))
		except TypeError:
			hours = 0

		try:
			minutes = int(re.search(r'Битва семи замков через ?((\d+)ч\.)?( (\d+) ?(мин\.|минуты|минуту))?!', event.raw_text).group(4))
		except TypeError:
			minutes = 0
...
async def attack_corovan():
	await client.send_message(game_id, MyHero.quest_button)
	await asyncio.sleep(random.randint(1, 3))
	await client.send_message(game_id, MyHero.quests_button_list['corovan'])
...
async def get_admin_message(event):
	if event.raw_text == 'help':
		...
		await client.send_message(admin_id, 'Bot disabled')
